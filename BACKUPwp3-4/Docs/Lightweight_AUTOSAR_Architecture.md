...
- `App`: application logic and cyclic runnable.
  ...
  - `App.c` periodically samples ADC, updates LCD1602, handles UART commands, and drives AT24C02 read/write jobs.
...
Application logic should call `Rte` APIs rather than directly including `Serial`, `Lcd1602`, `At24c02`, or `Iic`.
...
`App_MainFunction -> Rte_MainFunction -> Iic/At24c02/Lcd1602 state machines -> App logic`
...
- `Rte_Init()` initializes `Serial`, `Adc`, `Lcd1602`, `At24c02`, and `Timer2`.
...
- Configuration headers such as `Uart_Cfg.h`, `Adc_Cfg.h`, `Timer2_Cfg.h`, and `At24c02_Cfg.h` hold board-specific values rather than public module metadata.
