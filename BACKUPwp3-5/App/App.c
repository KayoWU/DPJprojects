...
/* 把4bit数值转成十六进制字符。 */
static uint8 App_ToHexChar(uint8 value)
{
...
/* LCD第二行显示红外遥控按键码，例如 IR KEY:0x44。 */
static void App_SetIrLine(uint8 keyValue)
{
    App_ClearLine(App_Line2);
    App_Line2[0] = 'I';
    App_Line2[1] = 'R';
    ...
    App_Line2[7] = '0';
    App_Line2[8] = 'x';
    App_Line2[9] = App_ToHexChar((uint8)(keyValue >> 4));
    App_Line2[10] = App_ToHexChar(keyValue);
    App_Line2Dirty = TRUE;
}
...
/* 红外接收处理：轮询RTE标志，收到新按键后显示到LCD第二行。 */
static void App_IrMainFunction(void)
{
    uint8 keyValue;
